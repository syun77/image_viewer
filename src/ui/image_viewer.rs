...
impl ImageViewer {
        ...
        egui::Window::new("")
            ...
            .show(ctx, |ui| {
                ...
                ui.allocate_new_ui(egui::UiBuilder::new().max_rect(image_rect), |ui| {
                    if self.is_loading {
                        Self::draw_loading_state(ui);
                    } else if let Some(error) = &self.error_message {
                        Self::draw_error_state(ui, error, current_image_file);
    ...
    fn draw_loading_state(ui: &mut egui::Ui) {
    ...
    fn draw_error_state(ui: &mut egui::Ui, error_message: &str, image_file: Option<&crate::core::file_scanner::ImageFile>) {
        let center = ui.available_rect_before_wrap().center();
        ui.painter().text(
            center,
            egui::Align2::CENTER_CENTER,
            &format!("読み込みエラー: {}", error_message),  // "Loading error: {}"
    ...
    fn log_error(&self, error_message: &str) {
        log::error!("ImageViewer Error: {}", error_message);
    }
}